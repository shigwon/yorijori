...
### Vue
1. 빠른 개발 속도 및 진입 장벽
- 프로젝트 경험이 적은 것을 고려함 -> 문법이 직관적
- 템플릿 기반 UI 작성이 용이함 
2. 모바일 최적화 용이 
- 반응형 구조를 위한 Vuetify와 TaliwindCSS와 같은 UI프레임워크와의 상성이 좋아
모바일 중심 UX 구현에 유리 -> 배달은 핸드폰과 밀접하기 때문
3. 컴포넌트 기반 설계
- 반복되는 UI 컴포넌트 단위로 분리하여 유지보수성과 확장성 확보 가능!
ex) 로봇 상태 표시, 음식 수령 버튼, 실시간 스트리밍 등!
### Kakao Maps JS API
1. 사용자에게 2D지도를 제공 -> 배달의 신뢰성 확보
2. 국내 서비스 특화 -> 한국의 골목, 산책로, 공원 구조 등에 디테일이 뛰어남
3. 프로젝트에 필요한 기능에 적합 -> 유료 전환 필요 없음
